  ...
  const loadMore = () => {
    ...
    startTransition(async () => {
      try {
        ...
        if (deduped.length === 0) { setDone(true); return; }
        ...
        {posts.map((post, i) => {
          ...
          return (
            <Link
              ...
              className={`card-md3 group flex flex-col ${i < 24 ? "fade-in-up" : ""}`}
              style={i < 24 ? { animationDelay: `${i * 40}ms` } : undefined}
            >
              <div className="relative overflow-hidden bg-surface-3">
                <img src={thumb} alt={post.title} className="h-full w-full object-cover transition-all duration-500 group-hover:scale-105" loading="lazy" style={{ aspectRatio: "3/4" }} />
                <div className="absolute bottom-0 left-0 right-0 h-1" style={{ backgroundColor: color }} />
              </div>
              <div className="flex flex-col gap-1 p-2.5">
                <h3 className="line-clamp-2 text-xs font-medium leading-snug">{post.title || t("post.untitled")}</h3>
