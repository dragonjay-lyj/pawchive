...
async function proxyRequest(req: NextRequest, params: { path: string[] }): Promise<NextResponse> {
  ...
  req.headers.forEach((v, k) => { const lo = k.toLowerCase(); if (lo === 'host' || lo === 'origin' || lo === 'referer') return; forwardHeaders.set(k, v); });
  forwardHeaders.set('Host', 'pawchive.st');
  const cookie = req.headers.get('cookie');
  if (cookie) forwardHeaders.set('Cookie', cookie);
  let body = undefined;
  if (req.method !== 'GET' && req.method !== 'HEAD') body = await req.text();
  const up = await fetch(targetUrl, { method: req.method, headers: forwardHeaders, body, redirect: 'follow' });
  const h = new Headers();
  up.headers.forEach((v, k) => { const lo = k.toLowerCase(); if (lo === 'transfer-encoding' || lo === 'content-encoding') return; h.set(k, v); });
  h.set('Access-Control-Allow-Origin', '*');
  h.set('Access-Control-Allow-Methods', 'GET, POST, DELETE, OPTIONS');
  h.set('Access-Control-Allow-Headers', 'Content-Type, Cookie');
  return new NextResponse(await up.arrayBuffer(), { status: up.status, statusText: up.statusText, headers: h });
}
